...
def log_add(message, overwrite=False):
    ...
    try:
        file = main_app_path + chr(92) + "Log.txt"
        ...
        msg = "WARNING: Módulo WsDatabase. " + \
              "No se pudo añadir mensaje al log." + chr(10) +\
              "MENSAJE:" + chr(10) +\
              message + chr(10)*2 +\
              "DETALLE:" + chr(10) + \
...
def crear_tablas(conn):
    try:
        ...
        msg = "ERROR CRÍTICO: Excepción módulo WsDatabase función crear_tablas." + chr(10)*2 +\
...
try:
    ...
    if TEST_MODE:
        my_path = os.getcwd()
        main_app_path = os.path.abspath(os.path.join(my_path, os.pardir))
        dbpath = main_app_path + r"\data.db"

    else:
        main_app_path = os.getcwd()
        dbpath = main_app_path + r"\data.db"
        my_path = db_main("SELECT * FROM PATH")[0][0]

    print("My path = '" + my_path + "'")
    print("Main app path = '" + main_app_path + "'")

except Exception as getpathserr:
    getpathsmsg = "ERROR CRÍTICO: Módulo WsDatabase. Imposible obtener path." + chr(10) +\
                  getpathserr.__class__.__name__ + " " + str(getpathserr)
    print(getpathsmsg)
    MessageBox(None, getpathsmsg, 'Error', ICON_ERROR)
    sys.exit()
